package com.easily.system.dict;

public class INNER {
    public static final String PACKAGE = "com.easily";

    public static final String FLOW_PACKAGE_VALUE = "com.carrot.core.flow";
    public static final String AOP_POOl_NAME = PACKAGE + ".factory.aop.AopPool";
    public static final String CONTROLLER_POOl_NAME = PACKAGE + ".factory.controller.ControllerPool";
    public static final String URLS_POOl_NAME = PACKAGE + ".factory.controller.UrlsPool";
    public static final String FILTER_POOl_NAME = PACKAGE + ".factory.filter.FilterPool";
    public static final String CONFIGURE_POOl_NAME = PACKAGE + ".factory.configure.ConfigurePool";
    public static final String EXCEPTION_POOl_NAME = "exceptionPool";
}
